...
export const About = memo(({ data }: AboutProps) => {
  ...
  const name         = data?.name         || "Mohammed Karab";
  const title        = data?.title        || "AI and Software Developer";
  const handle       = data?.handle       || "Im-Mohammed";
  const linkedin     = data?.linkedin     || "https://www.linkedin.com/in/mohammed-karab-ehtesham-469b83366";
  const bio          = data?.bio          || "";
  const typingLines  = data?.typing_lines || [
